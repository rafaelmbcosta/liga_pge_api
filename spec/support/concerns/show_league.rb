  ...
  module V1
    shared_examples 'show_league' do
      ...
      describe 'league_report_teams' do
        ...
        let(:battle) do
          Battle.new(first_id: team.id, second_id: nil,
                     first_points: 10, second_points: 4, draw: false,
                     first_win: false, second_win: false, round: round)
        end

        let(:teams) { [team] }
        let(:team_details) { [{ diff_points: 7, points: 2 }] }
        ...
        before do
          ...
          allow(model).to receive(:team_details).and_return(team_details)
        end
